  ...
  describe Api::V1::BooksController, type: :controller do
    ...
    describe 'POST /api/v1/books' do
      it 'creates a new Book if book does not already exist' do
        sign_in user
        post :create, params: {book: {title: book.title, author: book.author, isbn: book.isbn, url: book.url, image: book.image }, destination: { id: destination }}

        expect(response.status).to eq(201)
        res_body = JSON.parse(response.body)
        expect(res_body['book']['title']).to eq(book.title)
        expect(res_body['book']['author']).to eq(book.author)
